...
''';
  ...
  static Future<String?> generateTenantInfoPdf({
    ...
    try {
      ...
      // Helper function to calculate payment intervals
      List<Map<String, dynamic>> _calculatePaymentIntervals(Tenant t) {
        final intervals = <Map<String, dynamic>>[];
        double cumulativePaid = 0;
        DateTime currentCoverageStart = t.startDate;
        
        for (final payment in t.payments) {
          final lastDueDate = currentCoverageStart;
          cumulativePaid += payment.amount;
          
          // Calculate how many full months this payment covers
          final fullMonths = (cumulativePaid / t.monthlyRent).floor();
          final remainder = cumulativePaid - (fullMonths * t.monthlyRent);
          
          // Calculate new due date based on cumulative coverage
          final newDueDate = DateTime(
            t.startDate.year,
            t.startDate.month + fullMonths,
            t.startDate.day,
          );
          ...
          intervals.add({
            ...
            'outstandingBalance': remainder > 0.01 ? remainder : null, // Only show if > 1 cent
          });
          
          // Update coverage start for next payment
          currentCoverageStart = newDueDate;
        }
      ...
      pdf.addPage(
        pw.MultiPage(
          pageFormat: PdfPageFormat.a4,
          build: (context) => [
            // Large bold "TENANT INFORMATION" title
            pw.Text(
              'TENANT INFORMATION',
              style: pw.TextStyle(
                fontSize: 24,
            ...
            // House name, address, Room ID, and date in one row
            pw.Row(
              mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
            ...
            // Tenant details on left, image on right
            pw.Row(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                // Tenant details on left
                pw.Expanded(
                  flex: 2,
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      ...
                      if (tenant.phone != null && tenant.phone!.isNotEmpty)
                        pw.Text('Phone: ${tenant.phone}'),
                      if (tenant.email != null && tenant.email!.isNotEmpty)
                        pw.Text('Email: ${tenant.email}'),
                      if (tenant.jobTitle != null && tenant.jobTitle!.isNotEmpty)
                        pw.Text('Job: ${tenant.jobTitle}'),
                      pw.SizedBox(height: 8),
                ...
                // Tenant image on right (round borders)
                pw.Container(
                  width: 100,
                  height: 100,
                  decoration: pw.BoxDecoration(
            ...
            if (tenant.payments.isEmpty)
              ...
              pw.Table(
                ...
                children: [
                  ...
                  // Data rows
                  ..._calculatePaymentIntervals(tenant).map((interval) {
                    final payment = interval['payment'] as Payment;
                    final lastDue = interval['lastDueDate'] as DateTime;
                    final newDue = interval['newDueDate'] as DateTime;
                    final outstanding = interval['outstandingBalance'] as double?;
                    
                    return pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                          child: pw.Text(dateFmt.format(payment.date)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                          child: pw.Text('TZS ${currency.format(payment.amount)}'),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(8),
                          child: pw.Text(
                            '${dateFmt.format(lastDue)}\nto\n${dateFmt.format(newDue)}',
                        ...
                        pw.Padding(
                          ...
                          child: pw.Text(
                            outstanding != null && outstanding > 0
                                ? 'TZS ${currency.format(outstanding)}'
                                : '-',
                          ),
                        ),
                      ],
